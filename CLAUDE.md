...
| Bench | `WOWLIDATOR_BENCH=1 npm run bench` | the QA agent itself, scored on frozen proof bundles (`src/bench/`, `tests/fixtures/bench/`): **precision** of what it filed as a defect against a person's labels, the confusion of `diagnosis.origin` against the truth, and cost per case from the bundles' own token/turn fields; `flipRate` for stability across reruns. Pure — no model, no browser — and gated because it writes under `reports/bench/`: "should not run unasked". `--baseline <score.json>` prints the delta, so a change is judged on precision × cost, not cost alone. The tests in `tests/bench.test.ts` run always. |
...
- **The CLI is one entrypoint plus a package.** `src/cli.ts` keeps only what must stay at the bin path — the shebang, `main` (with the `ui` pre-`parseArgs` dispatch and the full option table), `cmdRecall`, and the test-facing re-exports (`EXIT`, `neverRan`, `suiteExit`, `exitCodeFor`, `classifyError`, `CaseOutcome`). Everything else lives in `src/cli/`: `usage.ts` (help text), `exit.ts` (the exit-code contract and suite verdicts), `options.ts` (`CliOptions` + flag parsing helpers), `runtime.ts` (model/logger builders, role gate), `artifacts.ts` (report/flow/Chrome side-effect helpers — `chromeStartedByUs` is module-private there on purpose; `growPool` adds Chromes on demand for personas), `run-cases.ts` (the shared suite loop), `case-plan.ts` (the pure scheduler: what may run beside what, `runQueue`, and the dependency gate `dependencyStanding` — a dependent waits for its prerequisite, runs only when it passed, and is `blocked` with the prerequisite's own outcome line otherwise; scenario-id references are linked by `linkDependencies` in `src/catalog/test-case-table.ts`; cycles and references to another sheet are printed once before the run), `sections.ts` + `data-locks.ts` (the parallelism rules: data sections per case, then step-level write locks; since 2026-09-05 an unknown agent-leg location defers its wildcard until the first governed write and locks the browser's live route, while deletes stay eagerly global), `suite-progress.ts` (the progress ledger that makes `--resume` possible), `catalog-live-report.ts` (the catalog report rewritten after every case), `pause.ts` (the suite-wide pause flag), `quota-hold.ts` (the suite-wide quota hold: when a claude-* provider serves any role, dispatch stops at `WOWLIDATOR_QUOTA_HOLD_PERCENT` of the account's session window — default 85 — and resumes when it reopens, so a full window never "finishes" the rest of the catalog on refusals; 2026-09-05, 243 cases lost that way), and `commands/{run,authoring,maintenance,go}.ts`. Dependencies flow strictly downward; `commands/go.ts` is the only cross-family edge. **A dependent's verdict is correctness, so the gate lives in the deterministic scheduler, never in the governor** — the governor only observes and explains a wait (see `src/orchestrator/CLAUDE.md`). **A case has a suite-owned ceiling** (2026-09-07): `--case-timeout <seconds>` overrides `WOWLIDATOR_CASE_TIMEOUT_MS`, then the measured 20-minute default; `0`/`off` disables it. Expiry aborts only that run, lets `runFlow`'s existing `finally` release every data lock, preserves the partial proof, and seals `blocked` so plain `--resume` tries it again; the pooled browser survives for the next lane.
